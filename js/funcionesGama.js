...
function postGama(){
    ...
    $.ajax({
        url:"http://144.22.132.80:8080/api/Gama/save",
        type:"POST",
        datatype:"JSON",
...
function putGama(idBotonActualizar){
    ...
    $.ajax({
        url:"http://144.22.132.80:8080/api/Gama/update",
        type:"PUT",
        datatype:"JSON",
...
function deleteGama(idBoton){
    ...
    Swal.fire({
        ...
        $.ajax({
            url:"http://144.22.132.80:8080/api/Gama/"+idBoton,
            type:"DELETE",
            datatype:"JSON",
...
"</tr>";    
    for(i=0;i<items.length;i++){
        ...
        myTable+="<td> <button onclick='putGama("+items[i].idGama+")'class='flex mx-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-lg'> Actualizar </button> " ;
        myTable+="<td> <button onclick='deleteGama("+items[i].idGama+")'class='flex mx-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-lg'> Borrar </button> " ;
        myTable+="</tr>";
    }
    myTable+="</table>";
